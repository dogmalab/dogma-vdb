...
fn collect_source_texts(root: &Path) -> Vec<(String, String)> {
    ...
    let extensions = ["rs", "md", "py", "toml", "json", "yaml", "sh", "txt", "js", "ts"];
...
fn main() {
    ...
    let embedder = match FastEmbedder::new() {
        ...
        Err(e) => {
            ...
            eprintln!("      Try: cargo run --release -p dogma-vdb-benchmarks --bin real-emb-bench");
            std::process::exit(1);
        }
    };
    ...
    for chunk in texts.chunks(EMBED_BATCH) {
        ...
        if embeddings.len() % (EMBED_BATCH * 10) == 0 {
            print!(".");
            use std::io::Write;
    ...
    let docs: Vec<Document> = sources
        ...
        .map(|((id, text), emb)| {
            Document::builder(id, text).embedding(emb).build()
        })
        .collect();

    // Split: N-QUERIES for queries, rest for index
    let n = docs.len();
    ...
    println!("      BF built in {:.3}s ({} docs)", bf_build.as_secs_f64(), bf.len());
    ...
    println!("[5/5] Building IVF-PQ (K-Means++, n_list={}, M={}, n_probe={})...",
    ...
    println!("================================================================");
    println!("  Dataset:      {} docs (hermes-agent), {} held-out queries",
             index_docs.len(), q_embeddings.len());
    println!("  Dimension:    {}", DIM);
    println!("  Model:        all-MiniLM-L6-v2 (ONNX / fastembed)");
    println!();
    println!("  ── Embedding Pipeline ──");
    println!("  Embed time:   {:.2}s ({:.0} docs/s)",
             embed_time.as_secs_f64(),
             index_docs.len() as f64 / embed_time.as_secs_f64().max(0.001));
    println!();
    println!("  ── Index Build ──");
    println!("  IVF-PQ build: {:.2}s", ivf_build.as_secs_f64());
    println!("  Config:       n_list={}, M={}, n_probe={}, K-Means++",
             N_LIST, M_SUB, N_PROBE);
    println!();
    println!("  ── Recall ──");
    println!("  Recall@1:     {:.1}%", total_recall_1 / nq * 100.0);
    println!("  Recall@10:    {:.1}%", total_recall_10 / nq * 100.0);
    println!();
    ...
    println!("  QPS:          {:.0}", 1_000_000.0 / mean_lat);
    println!("================================================================");
    ...
    if recall10 >= 0.70 {
        ...
        println!("⚠️  Recall@10 = {:.1}% (< 70% target). IVF-PQ + PQ ceiling.",
                 recall10 * 100.0);
