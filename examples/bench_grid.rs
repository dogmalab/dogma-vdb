...
fn bench_ivfpq(data: &TestData, ctx: &BenchContext, n_list: usize, m_sub: usize) -> IndexResult {
    let label = format!("IVF-PQ nlist={} M={}", n_list, m_sub);

    let n_probe = (n_list / 16).max(8);
...
fn main() {
            ...
            for &metric in METRICS {
                ...
                for &nl in IVF_NLIST_VALS {
                    ...
                    for &ms in IVF_M_SUB_VALS {
                        ...
                        eprintln!("Benchmarking IVF-PQ nlist={} M_sub={}...", nl, ms);
                        let r = bench_ivfpq(&data, &ctx, nl, ms);
                        append_json(json_path, &result_to_json(&r, n, dim, metric));
                        ivf_results.push(r);
                    }
                }
    ...
    fs::write("BENCHMARK.md", &master_md).ok();
    eprintln!(
        "\nDone! Results written to BENCHMARK.md and {}",
        json_path.display()
    );
}
