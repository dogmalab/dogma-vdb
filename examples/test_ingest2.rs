...
fn collect_files(root: &Path) -> Vec<std::path::PathBuf> {
    ...
    while let Some(dir) = dirs.pop() {
        ...
        for entry in entries.flatten() {
            let path = entry.path();
            if path.file_name().and_then(|s| s.to_str()).map_or(false, |s| s.starts_with('.')) { continue; }
...
fn main() {
    ...
    let root = std::env::args().nth(1).unwrap_or_else(|| "/home/arggil/Documents/DEV-WORKSPACE/hermes-agent".into());
    let files = collect_files(Path::new(&root));
    eprintln!("[TEST] Files: {}, RSS: {} MB", files.len(), rss_kb() / 1024);
    std::io::stderr().flush().ok();

    let chunker = SmartChunker::default();
    let binary_exts: &[&str] = &["png","jpg","jpeg","gif","ico","woff2","woff","ttf","otf","eot","pdf","zip","gz","pyc","mp3","mp4","webm"];
    ...
    eprintln!("[TEST] After {} files: {} docs, RSS: {} MB", 100, all_docs.len(), rss_kb() / 1024);
    std::io::stderr().flush().ok();
}
